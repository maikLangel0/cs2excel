...
use crate::models::price::{PriceType, Dopplers};

pub fn get_price(item_name: &str, prices: &Value, want: &PriceType, phase: Option<String>) -> Option<f64> {
        ...
        // One layer deep
            ...
            if let Some(doppler_phase) = phase {
                if let Some(p_two) = p_one.get("doppler") { 
                    ...
                    for finish in Dopplers::iter() {
                        
                        if finish.as_str() == doppler_phase {
                            if let Some(phase_price) = p_two.get( finish.as_str() ) {
